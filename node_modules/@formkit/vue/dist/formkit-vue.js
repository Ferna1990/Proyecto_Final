var FormKitVue = (function (exports, vue) {
    ...
    const FormKitKitchenSink = /* #__PURE__ */ vue.defineComponent({
        ...
        async setup(props) {
            ...
            const inputs = {};
            const promises = [];
            const coreInputPromises = inputList.core.map(async (input) => {
                const response = await fetchInputSchema(input);
                inputs[input] = response;
            });
            promises.push(...coreInputPromises);
            if (props.pro) {
                const proInputPromises = inputList.pro.map(async (input) => {
                    const response = await fetchInputSchema(input);
                    inputs[input] = response;
                });
            ...
            const inputComponents = inputKeys.map((input) => {
                const schemas = inputs[input];
                const schemaRenders = schemas.map((schema) => {
                    return vue.h('div', {
                ...
                return vue.h('div', { class: 'formkit-input-section' }, [
                    vue.h('span', { class: 'formkit-input-type' }, input),
                    vue.h('div', { class: 'formkit-specimen-group' }, schemaRenders),
                ]);
            });
            return () => {
                return vue.h('div', {
