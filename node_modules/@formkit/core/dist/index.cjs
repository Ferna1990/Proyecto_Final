...
function reset(id, resetTo) {
    ...
    if (node) {
        ...
        const resetValue = utils.cloneAny(resetTo);
        if (resetTo && !utils.empty(resetTo)) {
            node.props.initial = utils.isObject(resetValue) ? utils.init(resetValue) : resetValue;
        ...
        node.input(utils.empty(resetValue) && resetValue ? resetValue : initial(node), false);
        ...
        if (isDeepReset) {
            node.walk((child) => {
                child.props.initial = utils.isObject(child.value)
                    ? utils.init(child.value)
                    : child.value;
...
function input(node, context, value, async = true) {
    ...
    if (node.isCreated &&
        node.type === 'input' &&
        utils.eq(context._value, context.value)) {
...
function hydrate(node, context) {
    ...
    context.children.forEach((child) => {
        ...
        if (child.name in _value) {
            ...
            if (!child.isSettled ||
                (!utils.isObject(childValue) && utils.eq(childValue, child._value)))
                return;
 ...
 * @internal
 */
const FORMKIT_VERSION = '1.3.0-c4d1235';
